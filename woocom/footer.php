...
<?php if ( ! ( function_exists( 'is_checkout' ) && is_checkout() ) ) : ?>
		...
		.premium-footer-contact-icon {
			...
			border: 1px solid color-mix(in srgb, var(--color-primary, #2563EB) 10%, transparent);
			display: flex;
			align-items: center;
			justify-content: center;
			color: var(--color-primary, #2563EB);
...
<?php if ( get_option( 'woocom_setup_complete' ) === '1' && function_exists( 'WC' ) && WC()->cart && ! ( function_exists( 'is_checkout' ) && is_checkout() ) ) : ?>
<?php
...
$show_cart_floating_widget       = false;
?>
<!-- Sticky Cart Widget -->
...
<script>
    ...
    if (window.jQuery) {
        jQuery(document.body).on('added_to_cart', function(event, fragments, cart_hash, $button) {
            if (window.woocom_is_updating_qty) return;
    ...
    (function() {
        function openCart(isAutomatic) {
            ...
            var dropdown = document.getElementById('woocom-mini-cart-dropdown');
            if (dropdown) {
        ...
        window.woocomOpenCart = openCart;
        window.woocomCloseCart = closeCart;

        var bind = function() {
            ['cart-drawer-open-mobile', 'cart-drawer-open-bottom', 'cart-drawer-open-sticky'].forEach(function(id) {
        ...
        document.addEventListener('click', function(e) {
            var btn = e.target.closest('.woocom-custom-add-to-cart, .ajax_add_to_cart');
            if (!btn) return;
            e.preventDefault();
            e.stopPropagation();
            ...
            .then(function(data) {
                ...
                .then(function(cartData) {
                    ...
                    if (window.jQuery && jQuery(document.body).hasClass('woocommerce-checkout')) {
                        jQuery(document.body).trigger('update_checkout');
                    } else if (typeof window.woocomOpenCart === 'function') {
                        window.woocomOpenCart(true);
                    }
                });
            })
            .catch(function() {
