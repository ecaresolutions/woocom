function initWoocomTheme() {
    ...
    // Open Drawer function (Redirected to header mini-cart dropdown)
    function openCartDrawer() {
        ...
        console.log('woocom: openCartDrawer redirected to mini cart dropdown');
        const dropdown = document.getElementById('woocom-mini-cart-dropdown');
        if (dropdown) {
    ...
    [stickyWidget, mobileCartBtn, bottomCartBtn].forEach(btn => {
    ...
    window.cartUpdateStateAndCrossSells = cartUpdateStateAndCrossSells;
    ...
    if (window.jQuery) {
        jQuery(document.body).on('added_to_cart', function(event, fragments, cart_hash, $button) {
            ...
            fetch(`${woocom_ajax.ajax_url}?action=woocom_get_cart_data&nonce=${woocom_ajax.cart_nonce}`)
                .then(res => res.json())
                .then(data => {
                    cartUpdateStateAndCrossSells(data);
                    if (jQuery(document.body).hasClass('woocommerce-checkout')) {
                        // Refresh checkout page review order table and totals
                        jQuery(document.body).trigger('update_checkout');
                    } else {
                        // Automatically open mini cart dropdown
                        openCartDrawer();
                    }
                });
        });
    ...
    // Global function to open Quick View Modal (callable inline)
    window.woocomOpenQuickView = function(productId) {
        if (!productId) return;
    ...
    jQuery(document.body).on('click', '.woocom-quick-view-btn', function(e) {
        ...
        window.woocomOpenQuickView(productId);
    });
