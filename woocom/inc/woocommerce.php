...
if ( class_exists( 'WooCommerce' ) ) {
	 ...
	 */
	function woocom_get_cart_cross_sell_html( $limit = 10 ) {
		...
		// Cache cross-sell per unique cart composition and limit — 2-hour TTL.
		$cache_key = 'woocom_cross_sell_v3_' . md5( implode( ',', $product_ids ) . '_' . $limit );
...
/**
 * Automatically apply quantity-based bulk discounts to product prices in the cart/checkout
 */
add_action( 'woocommerce_before_calculate_totals', 'woocom_apply_quantity_bulk_discounts', 99, 1 );
function woocom_apply_quantity_bulk_discounts( $cart ) {
    if ( is_admin() && ! defined( 'DOING_AJAX' ) ) return;

    // Load active offer options (quantities & discount percentages) from theme settings
    $o1_qty      = intval( get_option( 'woocom_offer_1_qty', '2' ) );
    $o1_discount = intval( get_option( 'woocom_offer_1_discount', '6' ) );

    $o2_qty      = intval( get_option( 'woocom_offer_2_qty', '5' ) );
    $o2_discount = intval( get_option( 'woocom_offer_2_discount', '7' ) );

    $o3_qty      = intval( get_option( 'woocom_offer_3_qty', '10' ) );
    $o3_discount = intval( get_option( 'woocom_offer_3_discount', '10' ) );
    ...
    foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
        ...
        // Instantiate a fresh product object to always get the original un-discounted price
        $original_product = wc_get_product( $item_id );
        if ( ! $original_product ) {
            continue;
        }
        ...
        if ( $quantity >= $o3_qty && $o3_qty > 0 ) {
            $applied_discount = $o3_discount;
        } elseif ( $quantity >= $o2_qty && $o2_qty > 0 ) {
            $applied_discount = $o2_discount;
        } elseif ( $quantity >= $o1_qty && $o1_qty > 0 ) {
            $applied_discount = $o1_discount;
        }

        if ( $applied_discount > 0 ) {
            $discounted_price = $base_price * ( 1 - ( $applied_discount / 100 ) );
            $product->set_price( $discounted_price );
        }
    }
}

add_action( 'wp_footer', 'woocom_debug_cart_discount' );
function woocom_debug_cart_discount() {
    if ( ! class_exists( 'WooCommerce' ) || ! WC()->cart ) return;
    echo "\n<!-- CART DISCOUNT DEBUG:\n";
    echo "Active Offer 1: Qty=" . get_option('woocom_offer_1_qty', '2') . ", Disc=" . get_option('woocom_offer_1_discount', '6') . "%\n";
    echo "Active Offer 2: Qty=" . get_option('woocom_offer_2_qty', '5') . ", Disc=" . get_option('woocom_offer_2_discount', '7') . "%\n";
    echo "Active Offer 3: Qty=" . get_option('woocom_offer_3_qty', '10') . ", Disc=" . get_option('woocom_offer_3_discount', '10') . "%\n";
    foreach ( WC()->cart->get_cart() as $key => $item ) {
        $product = $item['data'];
        $item_id = ! empty( $item['variation_id'] ) ? $item['variation_id'] : $item['product_id'];
        $original_product = wc_get_product( $item_id );
        $orig_price = $original_product ? $original_product->get_price() : 'N/A';
        echo "Product ID: " . $item_id . ", Qty: " . $item['quantity'] . ", Original Price: " . $orig_price . ", Cart Item Price: " . $product->get_price() . "\n";
    }
    echo "-->\n";
}
