...
// Query published questions associated with this product
$qa_args = array(
...
if ( $qa_query->have_posts() ) {
    while ( $qa_query->have_posts() ) {
        ...
        $db_questions[] = array(
            ...
            'askedBy'  => get_post_meta( $q_id, '_asked_by', true ) ?: 'Anonymous',
            'date'     => get_the_date( 'm/d/Y' ),
            'answer'   => get_post_meta( $q_id, '_answer', true ) ?: '',
        );
    }
    wp_reset_postdata();
}
...
if ( woocom_is_grocery_product( $main_product_id ) ) {
                    ...
                    <div class="bg-white rounded-[6px] border border-gray-200 p-3 h-full flex flex-col group/card hover:shadow-md transition-shadow duration-300 relative overflow-hidden">
                        <div class="relative w-full pt-[100%] mb-2 bg-gray-50/30 rounded overflow-hidden">
                            <a href="<?php the_permalink(); ?>" class="block w-full h-full">
                                <img src="<?php echo esc_url( $image_url ); ?>" alt="<?php the_title_attribute(); ?>" class="w-full h-full object-cover scale-110 mx-auto">
                            </a>
                        </div>
                        <div class="flex-grow">
                        ...
                        <a href="?add-to-cart=<?php echo esc_attr( get_the_ID() ); ?>" data-product_id="<?php echo esc_attr( get_the_ID() ); ?>" class="woocom-custom-add-to-cart ajax_add_to_cart w-full border border-secondary/40 text-secondary hover:bg-secondary hover:text-white font-bold py-2 rounded-[6px] text-center transition-all duration-300 text-[14px] flex items-center justify-center gap-2 mt-auto">
                            Add To Cart
                        </a>
                    </div>
    ...
    // Load dynamic offers configuration from theme settings
    $offer1_badge    = get_option('woocom_offer_1_badge', 'MOST POPULAR');
    $offer1_qty      = intval(get_option('woocom_offer_1_qty', '2'));
    $offer1_discount = intval(get_option('woocom_offer_1_discount', '6'));

    $offer2_badge    = get_option('woocom_offer_2_badge', 'BEST VALUE');
    $offer2_qty      = intval(get_option('woocom_offer_2_qty', '5'));
    $offer2_discount = intval(get_option('woocom_offer_2_discount', '7'));

    $offer3_badge    = get_option('woocom_offer_3_badge', 'MOST SAVINGS');
    $offer3_qty      = intval(get_option('woocom_offer_3_qty', '10'));
    $offer3_discount = intval(get_option('woocom_offer_3_discount', '10'));
    $offer3_coupon   = get_option('woocom_offer_3_coupon', 'LARA10');

    // Dynamic bulk price calculations
    $price1_val = $sale_price * (1 - ($offer1_discount / 100));
    $price2_val = $sale_price * (1 - ($offer2_discount / 100));
    $price3_val = $sale_price * (1 - ($offer3_discount / 100));

    $price2 = '৳' . number_format( round( $price1_val ) );
    $price5 = '৳' . number_format( round( $price2_val ) );
    $price10 = '৳' . number_format( round( $price3_val ) );
    ...
    // Specs layout setup
    $specs = array();
    $db_specs = get_post_meta( $main_product_id, '_custom_product_specs', true );
        ...
        <div class="laracom-product-grid font-poppins">
            ...
            <div class="laracom-gallery-col">
                ...
                <div class="laracom-thumbnails-grid">
                    <?php
                    if ( is_array( $attachment_ids ) && count( $attachment_ids ) > 1 ) {
                        foreach ( $attachment_ids as $num => $attachment_id ) {
            ...
            <div class="lg:col-span-6 flex flex-col gap-5">
                ...
                <div class="laracom-price-card select-none flex items-center justify-between w-full">
                    <div class="flex items-center gap-2">
                        <span id="laracom-price-display" class="laracom-price-amount" style="color: var(--color-secondary, #F7A501) !important;">
                            <span class="taka-symbol" style="color: var(--color-secondary, #F7A501) !important;">৳</span><?php echo esc_html( number_format( $sale_price ) ); ?>
                        </span>
                        ...
                        <?php if ( $discount_pct > 0 ) : ?>
                            <span id="laracom-save-badge" class="laracom-save-badge" style="color: var(--color-secondary, #F7A501) !important; background-color: color-mix(in srgb, var(--color-secondary, #F7A501) 8%, #ffffff) !important;">
                    ...
                    <?php if ( $product->is_in_stock() ) : ?>
                        <span class="laracom-stock-badge in-stock">
                            In Stock
                        </span>
                    <?php else : ?>
                        <span class="laracom-stock-badge out-of-stock">
                            Out of Stock
                        </span>
                    <?php endif; ?>
                </div>
                ...
                <?php else : ?>
                    <div class="woocommerce-cart-form-container" style="display: none !important;">
                        <?php woocommerce_template_single_add_to_cart(); ?>
                    </div>
                <?php endif; ?>
                ...
                <div class="mt-3 font-poppins laracom-offers-wrapper">
                    ...
                    <div class="laracom-offers-grid select-none snap-x snap-mandatory no-scrollbar scroll-smooth">
                        ...
                        <div class="laracom-ticket-card snap-start">
                            <div class="laracom-ticket-badge whitespace-nowrap">
                                <?php echo esc_html($offer1_badge); ?>
                            </div>
                            <div 
                                onclick="setOfferQuantity(<?php echo $offer1_qty; ?>)"
                                class="ticket-card laracom-ticket-inner"
                                id="ticket-<?php echo $offer1_qty; ?>"
                            >
                                <div class="text-center">
                                    <div class="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-200">Buy <?php echo $offer1_qty; ?> or more</div>
                                </div>
                                ...
                                <div class="text-center w-full mt-auto">
                                    <div class="text-xs font-bold text-primary">Get <?php echo $offer1_discount; ?>% Off</div>
                        ...
                        <div class="relative pt-3 snap-start shrink-0 w-[calc(60%-10px)] sm:w-auto">
                            <div class="absolute top-0 left-1/2 -translate-x-1/2 bg-[#10B981] text-white text-[9px] font-black uppercase tracking-wider px-3.5 py-1 rounded-full shadow-sm z-10 whitespace-nowrap">
                                <?php echo esc_html($offer2_badge); ?>
                            </div>
                            <div 
                                onclick="setOfferQuantity(<?php echo $offer2_qty; ?>)"
                                class="ticket-card laracom-ticket-inner"
                                id="ticket-<?php echo $offer2_qty; ?>"
                            >
                                <div class="text-center">
                                    <div class="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-200">Buy <?php echo $offer2_qty; ?> or more</div>
                                </div>
                                <div class="laracom-ticket-divider">
                                ...
                                <div class="text-center w-full mt-auto">
                                    <div class="text-xs font-bold text-emerald-600 dark:text-emerald-400">Get <?php echo $offer2_discount; ?>% Off</div>
                        ...
                        <!-- Card 3 -->
                        <div class="relative pt-3 snap-start shrink-0 w-[calc(60%-10px)] sm:w-auto">
                            <div class="absolute top-0 left-1/2 -translate-x-1/2 bg-[#4F46E5] text-white text-[9px] font-black uppercase tracking-wider px-3.5 py-1 rounded-full shadow-sm z-10 whitespace-nowrap">
                                <?php echo esc_html($offer3_badge); ?>
                            </div>
                            <div 
                                onclick="setOfferQuantity(<?php echo $offer3_qty; ?>)"
                                class="ticket-card laracom-ticket-inner"
                                id="ticket-<?php echo $offer3_qty; ?>"
                            >
                                <div class="text-center">
                                    <div class="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-200">Buy <?php echo $offer3_qty; ?> or more</div>
                                </div>
                                <div class="laracom-ticket-divider">
                                ...
                                <div class="text-center w-full mt-auto">
                                    <div class="text-xs font-bold text-indigo-600 dark:text-indigo-400">Get <?php echo $offer3_discount; ?>% Off</div>
                                    <div class="flex items-center justify-center gap-1.5 mt-1">
                                        <span class="text-[11.5px] font-black text-slate-800 dark:text-slate-200">Code: <?php echo esc_html($offer3_coupon); ?></span>
                                        <button 
                                            type="button"
                                            onclick="copyCouponCode(event, '<?php echo esc_js($offer3_coupon); ?>')"
                ...
                <div class="laracom-tabs-col">
                    ...
                    <div class="laracom-tabs-bar">
                        ...
                        <button type="button" onclick="scrollToTabSection('questions')" id="tab-btn-questions" class="tab-header-btn w-full md:w-auto text-center px-1.5 md:px-5 py-2.5 rounded-lg text-[10px] sm:text-xs md:text-sm font-black border cursor-pointer truncate bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-202 border-slate-200 dark:border-slate-800 hover:border-primary/40">
                            Questions (<span id="qa-count-display"><?php echo count($db_questions); ?></span>)
                        </button>
                        <button type="button" onclick="scrollToTabSection('reviews')" id="tab-btn-reviews" class="tab-header-btn w-full md:w-auto text-center px-1.5 md:px-5 py-2.5 rounded-lg text-[10px] sm:text-xs md:text-sm font-black border cursor-pointer truncate bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-202 border-slate-200 dark:border-slate-800 hover:border-primary/40">
                            Reviews (<?php echo count($comments); ?>)
                        </button>
                    </div>
                    ...
                    <div id="specification" class="tab-content-sec scroll-mt-32 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 rounded-2xl p-5 md:p-8 shadow-sm animate-fade-in">
                        ...
                        <div class="flex flex-col border border-slate-100 dark:border-slate-850 rounded-xl overflow-hidden">
                            <?php foreach ( $specs as $sIdx => $spec ) : ?>
                                <div style="display: flex; align-items: flex-start; gap: 16px;" class="p-4 border-b border-slate-50 dark:border-slate-850/50 last:border-0 <?php echo ( $sIdx % 2 === 0 ) ? 'bg-slate-50/30 dark:bg-slate-900/10' : 'bg-white dark:bg-slate-900'; ?>">
                                    <div style="width: 30%; flex-shrink: 0;" class="text-xs md:text-sm font-semibold text-slate-500 dark:text-slate-400">
                                        <?php echo esc_html( $spec['label'] ); ?>
                                    </div>
                                    <div style="width: 70%;" class="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-100">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-5">
                ...
                while ( $related_query_bottom->have_posts() ) : $related_query_bottom->the_post();
                    ...
                    >
                        ...
                        <div class="aspect-square w-full overflow-hidden relative bg-slate-50/50 dark:bg-slate-955/20">
                            <img 
                            ...
                            <div class="mt-0.5">
                                ...
                                <?php else : ?>
                                    <a 
                                        ...
                                        class="woocom-custom-add-to-cart ajax_add_to_cart group w-full flex items-center justify-center gap-1.5 py-2 bg-primary-light hover:bg-primary text-primary hover:text-white text-[12px] font-bold rounded-full transition-all duration-300 active:scale-95"
                                    >
    ...
    <script type="text/javascript">
        ...
        const commentsList = <?php echo json_encode( array_map( function( $comment ) {
            ...
            $helpful = get_comment_meta( $comment->comment_ID, '_review_helpful', true );
            $unhelpful = get_comment_meta( $comment->comment_ID, '_review_unhelpful', true );
            ...
            return array(
                ...
                'verified'  => true,
                'helpful'   => intval($helpful) ?: 0,
                'unhelpful' => intval($unhelpful) ?: 0
            );
        }, $comments ) ); ?>;
        ...
        function updateOfferHighlightLaracom(qty) {
            ...
            var q3 = <?php echo $offer3_qty; ?>;
            var q2 = <?php echo $offer2_qty; ?>;
            var q1 = <?php echo $offer1_qty; ?>;
            if (qty >= q3) {
                var el = document.getElementById('ticket-' + q3);
                ...
                var el = document.getElementById('ticket-' + q2);
                if (el) { el.classList.add('border-primary', 'ring-2', 'ring-primary/10', 'scale-[1.02]', 'bg-primary-light'); el.classList.remove('border-orange-100'); }
            } else if (qty >= q1) {
                var el = document.getElementById('ticket-' + q1);
                if (el) { el.classList.add('border-primary', 'ring-2', 'ring-primary/10', 'scale-[1.02]', 'bg-primary-light'); el.classList.remove('border-orange-100'); }
            }
        }
        ...
        window.submitCustomQuestion = function(e) {
            ...
            const originalText = submitBtn ? submitBtn.innerText : 'Submit Question';
            if (submitBtn) {
            ...
            jQuery.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'submit_product_question',
                    product_id: currentProductId,
                    qa_name: name,
                    qa_email: email,
                    qa_content: content,
                    security: '<?php echo wp_create_nonce("submit_qa_nonce"); ?>'
                },
                success: function(response) {
                    if (submitBtn) {
                    ...
                    if (response.success) {
                        ...
                        if (typeof showToastNotification === 'function') {
                            showToastNotification(response.data || 'Failed to submit question. Please try again.');
                        } else {
                            alert(response.data || 'Failed to submit question. Please try again.');
                        }
                    }
                },
                error: function() {
                    ...
                    if (typeof showToastNotification === 'function') {
                        showToastNotification('Connection error. Please try again.');
                    } else {
        ...
        function initQuestionsSection() {
            const allQuestions = <?php echo wp_json_encode( $db_questions ); ?>;
            ...
            if (allQuestions.length === 0) {
                container.innerHTML = '<div class="text-center py-8 text-slate-400 dark:text-slate-500 font-medium">No questions asked yet. Be the first to ask a question!</div>';
                return;
            }
        ...
        function renderReviewsList() {
            ...
            if (sorted.length === 0) {
                container.innerHTML = '<div class="col-span-full text-center py-12 text-slate-400 dark:text-slate-500 font-medium">No reviews yet. Be the first to write a review!</div>';
        ...
        window.voteHelpful = function(idx, type, el) {
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        rev[type] = response.data.new_count;
                        const countSpan = el.querySelector('span');
                        if (countSpan) countSpan.innerText = rev[type];
                    }
                }
            });
        };
        ...
        function initLaracomProductScript() {
            ...
            jQuery(document).ready(function($) {
                ...
                function updateOfferHighlight(qty) {
                    ...
                    var q2 = <?php echo $offer2_qty; ?>;
                    var q1 = <?php echo $offer1_qty; ?>;
                    if (qty >= q3) $('#ticket-' + q3).addClass('border-primary ring-2 ring-primary/10 scale-[1.02] bg-primary-light').removeClass('border-orange-100');
                    else if (qty >= q2) $('#ticket-' + q2).addClass('border-primary ring-2 ring-primary/10 scale-[1.02] bg-primary-light').removeClass('border-orange-100');
                    else if (qty >= q1) $('#ticket-' + q1).addClass('border-primary ring-2 ring-primary/10 scale-[1.02] bg-primary-light').removeClass('border-orange-100');
                }
