...
?>
<div <?php wc_product_class( 'bg-white rounded-[4px] border border-gray-200 p-2 sm:p-3 h-full flex flex-col group/card hover:shadow-md transition-shadow duration-300 relative w-full overflow-hidden', $product ); ?>>
    ...
    <div class="flex-grow">
        ...
        <div class="flex items-center gap-1 sm:gap-1.5 mb-2 mt-0 flex-wrap">
            <span class="text-secondary font-bold text-[13px] sm:text-[15px]">
    ...
    elseif ( $product->is_type( 'simple' ) && $product->is_purchasable() && $product->is_in_stock() ) :
        echo sprintf(
            '<a href="%s" data-product_id="%s" class="add_to_cart_button ajax_add_to_cart w-full border border-secondary/40 text-secondary hover:bg-secondary hover:text-white font-bold py-1.5 sm:py-2.5 rounded-[4px] text-center transition-all duration-300 text-[13px] sm:text-[15px] flex items-center justify-center gap-1 sm:gap-2 mt-auto" rel="nofollow">%s %s</a>',
        ...
        echo sprintf(
            '<a href="%s" class="w-full border border-secondary/40 text-secondary hover:bg-secondary hover:text-white font-bold py-1.5 sm:py-2.5 rounded-[4px] text-center transition-all duration-300 text-[11px] sm:text-[14px] flex items-center justify-center gap-1 sm:gap-2 mt-auto">%s</a>',
